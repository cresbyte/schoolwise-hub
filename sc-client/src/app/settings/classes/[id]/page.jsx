...
function ClassDetailContent({ id }) {
...
function AttendanceTrendsTab({ classId }) {
  const [range, setRange] = useState(30);
  const end = new Date().toISOString().slice(0, 10);
  const start = new Date(Date.now() - range * 86400000).toISOString().slice(0, 10);

  const summary = useAsync(() => api.getAttendanceSummary(classId, start, end), [classId, range]);
  ...
  return (
    <Box>
      ...
      <DataState loading={summary.loading} error={summary.error} data={days} onRetry={summary.refetch} isEmpty={(d) => d.length === 0}>
        {() => (
          <>
                ...
                {days.map((day) => {
                  ...
                  return (
                    <TableRow key={day.date}>
                      ...
                      <TableCell align="right">{day.excused}</TableCell>
...
function PerformanceTab({ classId, gradeLevel }) {
