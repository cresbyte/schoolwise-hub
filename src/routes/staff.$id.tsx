...
export const Route = createFileRoute("/staff/$id")({
  head: () => ({ meta: [{ title: "Staff Profile — SchuleSmart" }] }),
  component: () => (
    <DashboardLayout>
      <StaffDetailPage />
    </DashboardLayout>
...
function StaffDetailPage() {
  ...
  return (
    <DataState loading={loading} error={error} data={data} onRetry={refetch}>
      {(s: Staff) => (
        <>
          <Button startIcon={<ArrowBackIcon />} onClick={() => navigate({ to: "/staff" })} sx={{ mb: 2 }}>
            Back to Staff
          </Button>
