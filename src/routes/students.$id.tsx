/**
 * Student detail page with tabbed sections.
 * @module routes/students.$id
 */
...
export const Route = createFileRoute("/students/$id")({
  head: () => ({ meta: [{ title: "Student Profile — SchuleSmart" }] }),
  component: () => (
    <DashboardLayout>
      <StudentDetail />
    </DashboardLayout>
...
function StudentDetail() {
  ...
  return (
    <DataState loading={loading} error={error} data={data} onRetry={refetch}>
      {(s: Student) => (
        <>
          <Button startIcon={<ArrowBackIcon />} onClick={() => navigate({ to: "/students" })} sx={{ mb: 2 }}>
            Back to Students
          </Button>
          <Card sx={{ mb: 2 }}>
            <CardContent>
      ...
      {(i) => (
        <>
          ...
          <Table size="small" sx={{ mb: 3 }}>
            ...
            <TableBody>
              {i.items.map((it) => <TableRow key={it.name}><TableCell>{it.name}</TableCell><TableCell align="right">{formatKES(it.amount)}</TableCell></TableRow>)}
            </TableBody>
          </Table>
