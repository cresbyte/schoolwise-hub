...
export const Route = createFileRoute("/classes/$id")({
  head: () => ({ meta: [{ title: "Class Detail — SchuleSmart" }] }),
  component: () => (
    <DashboardLayout>
      <ClassDetailPage />
    </DashboardLayout>
...
function ClassDetailPage() {
  ...
  return (
    <DataState loading={classData.loading} error={classData.error} data={classData.data} onRetry={classData.refetch}>
      {(cls) => (
        <>
          <Button startIcon={<ArrowBackIcon />} onClick={() => navigate({ to: "/classes" })} sx={{ mb: 2 }}>
            Back to Classes
          </Button>
          <Card sx={{ mb: 2 }}>
            <CardContent>
          ...
          <TableBody>
            {list.map((s: Student, i) => (
              <TableRow key={s.id} hover>
                ...
                <TableCell>
                  <Button size="small" component={Link} to={`/students/${s.id}`}>View</Button>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
...
function TimetableTab({ classId }: { classId: string }) {
  ...
  list.forEach((slot) => {
    if (grid[slot.startTime]) grid[slot.startTime][slot.dayOfWeek] = slot;
  });
          ...
          <>
            ...
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={data.subjectStats}>
                ...
                <XAxis dataKey="subject" fontSize={11} tick={{ angle: -25 }} height={50} />
