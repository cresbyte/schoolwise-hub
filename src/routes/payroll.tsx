...
function PayrollPage() {
  ...
  return (
    <>
        ...
        actions={
          <RoleGuard permission="payroll.process">
            {list.length === 0 ? (
              ...
              <>
                <Button variant="outlined" onClick={approve} disabled={busy}>Approve</Button>
      ...
      <Card>
        <DataState loading={payroll.loading} error={payroll.error} data={list} onRetry={payroll.refetch} isEmpty={(d) => d.length === 0} emptyMessage="Payroll not yet run for this month">
          {() => (
